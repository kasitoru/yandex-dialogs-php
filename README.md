...
`public function get_some_text(string $pattern, string $text=null): array`

Получает часть текста на основе заданного шаблона. Метод возвращает ассоциативный массив, в котором в качестве ключей выступают имена паттернов, а значения заполнены найденными результатами. Если поиск не удался (оригинальный текст не подходит под указанный шаблон, либо шаблон составлен с ошибкой), то метод возвращает false.

`$pattern` - Шаблон поиска, оборачивается фигурными скобками. Подробнее см. в примере ниже. Строка. Обязательный параметр.
